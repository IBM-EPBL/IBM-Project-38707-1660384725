...
def send_mail():
    ...
    query = "select * from inventory_table where username='" +username+ "' and qty<threshold;"

    lt = []
    stmt = ibm_db.exec_immediate(conn,query)
    while ibm_db.fetch_row(stmt) != False:
        lt.append([ibm_db.result(stmt, 0),ibm_db.result(stmt, 1),str(ibm_db.result(stmt, 2)),ibm_db.result(stmt, 3),str(ibm_db.result(stmt, 5))])
    if not lt:
        print("No data found")
    ...
    mid = ""
    for l in lt:
        mid += "<tr><td>"+l[0]+"</td><td>"+l[1]+"</td><td>"+l[2]+"</td><td>"+l[3]+"</td><td>"+l[4]+"</td></tr>"
